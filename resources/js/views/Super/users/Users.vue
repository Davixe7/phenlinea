<template>
  <div id="users">
    ...
    <table class="table table-responsive" v-if="users.length">
      ...
      <tbody>
        <tr v-for="user in users">
          ...
          <td class="text-right">
            <div class="btn-group">
              <a href="#" class="btn btn-sm btn-secondary" @click="editUser(user)">
                <i class="material-icons">edit</i></a>
              <a href="#" class="btn btn-sm btn-secondary" @click="deleteUser(user.id)">
    ...
    <!-- Modal -->
    <div ref="UsersModal" class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
export default {
  ...
  methods:{
    ...
    updateUser(user){
      ...
      this.$toasted.success('Usuario actualizado exitosamente', {position:'bottom-left'});
    },
    deleteUser(id){
      if( window.confirm('¿Seguro que quieres eliminar al usuario?') ){
        axios.delete('/admin/users/' + id).then((response)=>{
          ...
          this.$toasted.success('Usuario eliminado exitosamente', {position:'bottom-left'});
        })
        .catch((error)=>{
    ...
    appendUser(user){
      ...
      this.$toasted.success('Usuario creado exitosamente', {position: 'bottom-left'})
    }
  },
  mounted(){
